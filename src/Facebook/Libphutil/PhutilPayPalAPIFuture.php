...
final class PhutilPayPalAPIFuture extends \Facebook\Libphutil\FutureProxy {
  ...
  protected function getProxiedFuture() {
    if (!$this->future) {
      ...
      $this->future = \Facebook\Libphutil\Functions\utils::id(new \Facebook\Libphutil\HTTPSFuture($this->getHost(), $params))
        ->setMethod('POST');
    }
  ...
  protected function didReceiveResult($result) {
    ...
    if (\Facebook\Libphutil\Functions\utils::idx($dict, 'ACK') !== 'Success') {
      throw new \Exception(
