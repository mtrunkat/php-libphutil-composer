...
final class AphrontMySQLDatabaseConnection
  ...
  protected function connect() {
    ...
    if ($pass instanceof \Facebook\Libphutil\PhutilOpaqueEnvelope) {
    ...
    if (!$conn) {
      $errno = mysql_errno();
      $error = mysql_error();
      throw new \Facebook\Libphutil\AphrontQueryConnectionException(
  ...
  public function executeParallelQueries(
    array $queries,
    array $conns = array()) {
    \Facebook\Libphutil\Functions\utils::assert_instances_of($conns, '\Facebook\Libphutil\AphrontMySQLDatabaseConnection');

    $map = array();
    $is_write = false;
    foreach ($queries as $id => $query) {
      $is_write = $is_write || $this->checkWrite($query);
      $conn = \Facebook\Libphutil\Functions\utils::idx($conns, $id, $this);
    ...
    $err_pos = 0;
    foreach ($queries as $id => $query) {
      $errno = \Facebook\Libphutil\Functions\utils::idx(\Facebook\Libphutil\Functions\utils::idx($map, 'errno', array()), $err_pos);
      $err_pos++;
      if ($errno) {
        try {
