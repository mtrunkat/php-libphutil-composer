 ...
 * @group aws
 */
abstract class PhutilAWSFuture extends \Facebook\Libphutil\FutureProxy {
  ...
  protected function getProxiedFuture() {
    if (!$this->future) {
      ...
      $uri = new \Facebook\Libphutil\PhutilURI('http://'.$this->getHost().'/');
      $uri->setQueryParams($params);

      $this->future = new \Facebook\Libphutil\HTTPFuture($uri);
    }
  ...
  protected function didReceiveResult($result) {
    ...
    if ($status->isError() || !$xml) {
      ...
      throw new \Facebook\Libphutil\PhutilAWSException($status->getStatusCode(), $params);
    }
