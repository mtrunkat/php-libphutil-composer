...
final class PhutilOAuth1Future extends \Facebook\Libphutil\FutureProxy {
  ...
  public function __construct($uri, $data = array()) {
    $this->uri = new \Facebook\Libphutil\PhutilURI((string)$uri);
    $this->data = $data;
    $this->setProxiedFuture(new \Facebook\Libphutil\HTTPSFuture($uri, $data));
  }
  ...
  private function getOAuth1Headers() {
    if (!$this->nonce) {
      $this->nonce = \Facebook\Libphutil\Filesystem::readRandomCharacters(32);
    }
    if (!$this->timestamp) {
      $this->timestamp = time();
    }
  ...
  private function sign(array $params) {
    ...
    $sign_uri->setProtocol(\Facebook\Libphutil\Functions\utf8::phutil_utf8_strtolower($sign_uri->getProtocol()));
    ...
    $method = rawurlencode(\Facebook\Libphutil\Functions\utf8::phutil_utf8_strtoupper($this->method));
