...
class PhutilTypeSpec {
  ...
  public function check($value, $name = null) {
    switch ($this->type) {
      case 'int':
        if (!is_int($value)) {
          throw new \Facebook\Libphutil\PhutilTypeCheckException($this, $value, $name);
        }
        break;
      case 'float':
        if (!is_float($value)) {
      ...
      case 'regex':
        ...
        if ($ok === false) {
          throw new \Facebook\Libphutil\PhutilTypeCheckException($this, $value, $name, $err);
        }
        break;
      case 'null':
        if (!is_null($value)) {
      ...
      case 'list':
        ...
        } catch (\Facebook\Libphutil\PhutilTypeCheckException $ex) {
          throw new \Facebook\Libphutil\PhutilTypeCheckException($this, $value, $name);
        }
        break;
      case 'map':
  ...
  public static function checkMap(array $values, array $types) {
    ...
    if ($extra) {
      throw new \Facebook\Libphutil\PhutilTypeExtraParametersException($extra);
    }
    ...
    if ($missing) {
      throw new \Facebook\Libphutil\PhutilTypeMissingParametersException($missing);
    }
  ...
  public static function getCommonParentClass($class_a, $class_b) {
    ...
    return \Facebook\Libphutil\Functions\utils::head(array_intersect($ancestors_a, $ancestors_b));
  }
  ...
  public static function didReduceTokens($rule, $production, array $tokens) {
    switch ($rule) {
      ...
      case 'type':
        ...
        return $tokens[0];
      case 'basic_type':
        $obj = new \Facebook\Libphutil\PhutilTypeSpec();
        $obj->type = $tokens[0][1];
        return $obj;
      case 'or_type':
        $l = $tokens[0];
  ...
  private static function getLexer() {
    static $lexer;
    if (!$lexer) {
      $lexer = new \Facebook\Libphutil\PhutilTypeLexer();
    }
    return $lexer;
  }
  ...
  private static function parseTokens(array $tokens, $callback) {
    ...
    return \Facebook\Libphutil\PhutilParserGenerator::parseTokensWithTables(
      array (
        0 =>
        array (
          'opt' =>
