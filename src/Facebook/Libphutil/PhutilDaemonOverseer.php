...
final class PhutilDaemonOverseer {
  ...
  public function __construct(array $argv) {
    \Facebook\Libphutil\PhutilServiceProfiler::getInstance()->enableDiscardMode();
...
EOHELP
    ...
    if ($this->phddir) {
      ...
      \Facebook\Libphutil\Filesystem::writeFile(
  ...
  public function run() {
    ...
    $argv = array();
    $argv[] = \Facebook\Libphutil\Functions\csprintf::csprintf('exec ./exec_daemon.php %s', $this->daemon);
    foreach ($this->argv as $k => $arg) {
      $argv[] = \Facebook\Libphutil\Functions\csprintf::csprintf('%s', $arg);
    }
    $argv[] = '--';
    ...
    while (true) {
      ...
      $future = new \Facebook\Libphutil\ExecFuture('%C', $command);
  ...
  public function didReceiveTerminalSignal($signo) {
    ...
    $signame = \Facebook\Libphutil\Functions\execx::phutil_get_signal_name($signo);
    if ($signame) {
  ...
  public static function findRunningDaemons() {
    ...
    list($err, $processes) = \Facebook\Libphutil\Functions\execx::exec_manual('ps -o pid,command -a -x -w -w -w');
    if ($err) {
      return $results;
    }
  ...
  private function generateDaemonID() {
    return substr(getmypid().':'.\Facebook\Libphutil\Filesystem::readRandomCharacters(12), 0, 12);
  }
  ...
  private function dispatchEvent($type, array $params = array()) {
    $data = array(
      '\Facebook\Libphutil\Functions\utils::id' => $this->daemonID,
      'daemonClass' => $this->daemon,
      'childPID' => $this->childPID,
    ) + $params;

    $event = new \Facebook\Libphutil\PhutilEvent($type, $data);
