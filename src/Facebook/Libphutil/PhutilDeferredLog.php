 ...
 *
 *   $log = new \Facebook\Libphutil\PhutilDeferredLog('/path/to/access.log', "[%T]\t%u");
 *
...
/* -(  Logging  )------------------------------------------------------------ */
   ...
   * later:
   *
   *   $log = new \Facebook\Libphutil\PhutilDeferredLog('/some/file.log', '[%T] %u');
   *
  ...
  public function getData($key, $default = null) {
    return \Facebook\Libphutil\Functions\utils::idx($this->data, $key, $default);
  }
  ...
  public function write() {
    ...
    if ($this->file !== null) {
      ...
      if ($ok === false) {
        ...
        if ($this->failQuietly) {
          \Facebook\Libphutil\Functions\phlog::phlog($message);
        } else {
...
/* -(  Internals  )---------------------------------------------------------- */
  ...
  private function format() {
    ...
    $result = '';
    $saw_percent = false;
    foreach (\Facebook\Libphutil\Functions\utf8::phutil_utf8v($this->format) as $c) {
      if ($saw_percent) {
        $saw_percent = false;
