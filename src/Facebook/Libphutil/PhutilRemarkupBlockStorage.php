  ...
  public function restore($corpus, $text_mode = false) {
    if ($this->map) {
      if ($text_mode) {
        ...
        $corpus = \Facebook\Libphutil\Functions\render::phutil_safe_html(str_replace(
          array_reverse(array_keys($this->map)),
          array_map('\Facebook\Libphutil\Functions\render::phutil_escape_html', array_reverse($this->map)),
          \Facebook\Libphutil\Functions\render::phutil_escape_html($corpus)));
      }
    }
    return $corpus;
  }
