...
final class PhagePHPAgentBootloader extends \Facebook\Libphutil\PhageAgentBootloader {
  ...
  public function getBootCommand() {
    ...
    return \Facebook\Libphutil\Functions\csprintf::csprintf(
      'php -r %s',
  ...
  private function buildBootSequence() {
    if (!$this->bootSequence) {
      ...
      $main_sequence = new \Facebook\Libphutil\PhutilBallOfPHP();
      $root = \Facebook\Libphutil\Functions\moduleutils::phutil_get_library_root('phutil');
      foreach ($files as $file) {
        $main_sequence->addFile($root.'/'.$file);
      }
      $main_sequence->addText('\Facebook\Libphutil\Functions\utils::id(new \Facebook\Libphutil\PhagePHPAgent($I))->execute();');
      $main_length = strlen($main_sequence->toString());

      $boot_sequence = new \Facebook\Libphutil\PhutilBallOfPHP();
      $boot = '
        $length = '.$main_length.';
        $buffer = "";
