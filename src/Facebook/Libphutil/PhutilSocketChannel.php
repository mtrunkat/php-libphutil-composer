 ...
 *   $channel = new \Facebook\Libphutil\PhutilSocketChannel($socket);
 *
 * Using stdin/stdout:
 *
 *   $channel = new \Facebook\Libphutil\PhutilSocketChannel(
 *     fopen('php://stdin', 'r'),
 *     fopen('php://stdout', 'w'));
 *
...
/* -(  Construction  )------------------------------------------------------- */
   ...
   *
   *   list($x, $y) = \Facebook\Libphutil\PhutilSocketChannel::newChannelPair();
   *
   * @task construct
   */
  public static function newChannelPair() {
    ...
    $domain = \Facebook\Libphutil\Functions\utils::phutil_is_windows() ? STREAM_PF_INET : STREAM_PF_UNIX;
    $pair = stream_socket_pair($domain, STREAM_SOCK_STREAM, STREAM_IPPROTO_IP);
    if (!$pair) {
      throw new \Exception("stream_socket_pair() failed!");
    }

    $x = new \Facebook\Libphutil\PhutilSocketChannel($pair[0]);
    $y = new \Facebook\Libphutil\PhutilSocketChannel($pair[1]);
  ...
  protected function writeBytes($bytes) {
    ...
    $len = \Facebook\Libphutil\Functions\utils::phutil_fwrite_nonblocking_stream($socket, $bytes);
    if ($len === false) {
