 ...
 *
 * To use \Facebook\Libphutil\PhutilErrorHandler, which will enhance the messages printed to the
 * PHP error log, just initialize it:
 *
 *    \Facebook\Libphutil\PhutilErrorHandler::initialize();
 *
 ...
 *
 *    \Facebook\Libphutil\PhutilErrorHandler::setErrorListener($some_callback);
 *
...
final class PhutilErrorHandler {
  ...
  const ERROR       = 'error';
  const PHLOG       = '\Facebook\Libphutil\Functions\phlog::phlog';
...
/* -(  Configuring Error Dispatch  )----------------------------------------- */
  ...
  public static function initialize() {
    ...
    set_error_handler(array('\Facebook\Libphutil\PhutilErrorHandler', 'handleError'));
    set_exception_handler(array('\Facebook\Libphutil\PhutilErrorHandler', 'handleException'));
  }
...
/* -(  Trapping Errors  )---------------------------------------------------- */
   ...
   *
   * @param \Facebook\Libphutil\PhutilErrorTrap Trap to add.
   * @return void
   * @task trap
   */
  public static function addErrorTrap(\Facebook\Libphutil\PhutilErrorTrap $trap) {
    $key = $trap->getTrapKey();
    self::$traps[$key] = $trap;
  }
   ...
   *
   * @param \Facebook\Libphutil\PhutilErrorTrap Trap to remove.
   * @return void
   * @task trap
   */
  public static function removeErrorTrap(\Facebook\Libphutil\PhutilErrorTrap $trap) {
    $key = $trap->getTrapKey();
...
/* -(  Internals  )---------------------------------------------------------- */
  ...
  /**
   * Determine if \Facebook\Libphutil\PhutilErrorHandler has been initialized.
   *
  ...
  public static function formatStacktrace($trace) {
    ...
    foreach ($trace as $key => $entry) {
      ...
      $line .= \Facebook\Libphutil\Functions\utils::idx($entry, 'function', '');

      if (isset($entry['args'])) {
        $args = array();
        foreach ($entry['args'] as $arg) {
          $args[] = \Facebook\Libphutil\PhutilReadableSerializer::printShort($arg);
        }
        $line .= '('.implode(', ', $args).')';
      }
  ...
  public static function dispatchErrorMessage($event, $value, $metadata) {
    ...
    switch ($event) {
      case \Facebook\Libphutil\PhutilErrorHandler::ERROR:
        ...
        break;
      case \Facebook\Libphutil\PhutilErrorHandler::EXCEPTION:
        $messages = array();
        $current = $value;
        do {
        ...
        break;
      case \Facebook\Libphutil\PhutilErrorHandler::PHLOG:
        $default_message = sprintf(
          '[%s] PHLOG: %s at [%s:%d]',
          $timestamp,
          \Facebook\Libphutil\PhutilReadableSerializer::printShort($value),
          $metadata['file'],
          $metadata['line']);
        ...
        error_log($default_message);
        break;
      case \Facebook\Libphutil\PhutilErrorHandler::DEPRECATED:
        $default_message = sprintf(
