...
abstract class AASTNode {
  ...
  public function __construct($id, array $data, \Facebook\Libphutil\AASTTree $tree) {
    $this->id = $id;
    $this->typeID = $data[0];
    if (isset($data[1])) {
      $this->l = $data[1];
  ...
  public function getChildByIndex($index) {
    // NOTE: Microoptimization to avoid calls like array_values() or \Facebook\Libphutil\Functions\utils::idx().
  ...
  public function selectTokensOfType($type_name) {
    if (isset($this->tokenCache)) {
      return \Facebook\Libphutil\Functions\utils::idx($this->tokenCache, $type_name, array());
    } else {
      $result = array();
      foreach ($this->getTokens() as $id => $token) {
  ...
  public function selectDescendantsOfType($type_name) {
    ...
    return \Facebook\Libphutil\AASTNodeList::newFromTreeAndNodes($this->tree, $nodes);
  }
  ...
  public function getSemanticString() {
    ...
    return implode('', \Facebook\Libphutil\Functions\utils::mpull($tokens, 'getValue'));
  }
  ...
  public function getLineNumber() {
    return \Facebook\Libphutil\Functions\utils::idx($this->tree->getOffsetToLineNumberMap(), $this->getOffset());
  }
