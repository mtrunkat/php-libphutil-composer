...
/* -(  Parsing Arguments  )-------------------------------------------------- */
  ...
  public function parsePartial(array $specs) {
    ...
    $specs_by_name  = \Facebook\Libphutil\Functions\utils::mpull($specs, null, 'getName');
    $specs_by_short = \Facebook\Libphutil\Functions\utils::mpull($specs, null, 'getShortAlias');
    ...
    for ($ii = 0; $ii < $len; $ii++) {
      ...
      if ($map) {
        ...
        if (isset($map[$arg])) {
          ...
          if ($val !== null) {
            if ($param_name === null) {
              throw new \Facebook\Libphutil\PhutilArgumentUsageException(
  ...
  public function parseFull(array $specs) {
    ...
    if (count($this->argv)) {
      $arg = \Facebook\Libphutil\Functions\utils::head($this->argv);
      throw new \Facebook\Libphutil\PhutilArgumentUsageException(
  ...
  public function parse(array $specs) {
    ...
    } catch (\Facebook\Libphutil\PhutilArgumentUsageException $ex) {
  ...
  public function parseWorkflowsFull(array $workflows) {
    \Facebook\Libphutil\Functions\utils::assert_instances_of($workflows, '\Facebook\Libphutil\PhutilArgumentWorkflow');
    ...
    foreach ($workflows as $workflow) {
      ...
      if ($name === null) {
        throw new \Facebook\Libphutil\PhutilArgumentSpecificationException(
    ...
    if (empty($this->workflows[$flow])) {
      ...
      throw new \Facebook\Libphutil\PhutilArgumentUsageException($ex_msg);
    }
    ...
    if ($this->showHelp) {
      ...
      $help_flow = \Facebook\Libphutil\Functions\utils::idx($this->workflows, 'help');
      if ($help_flow) {
  ...
  public function parseStandardArguments() {
    ...
    if ($this->getArg('trace')) {
      \Facebook\Libphutil\PhutilServiceProfiler::installEchoListener();
    }

    if ($this->getArg('no-ansi')) {
      \Facebook\Libphutil\PhutilConsoleFormatter::disableANSI(true);
    }

    if ($this->getArg('ansi')) {
      \Facebook\Libphutil\PhutilConsoleFormatter::disableANSI(false);
    }
    ...
    if ($recon) {
      $remote_console = \Facebook\Libphutil\PhutilConsole::newRemoteConsole();
      $remote_console->beginRedirectOut();
      \Facebook\Libphutil\PhutilConsole::setConsole($remote_console);
    } else if ($this->getArg('trace')) {
      $server = new \Facebook\Libphutil\PhutilConsoleServer();
      $server->setEnableLog(true);
      $console = \Facebook\Libphutil\PhutilConsole::newConsoleForServer($server);
      \Facebook\Libphutil\PhutilConsole::setConsole($console);
    }
...
/* -(  Reading Arguments  )-------------------------------------------------- */
  ...
  public function getArg($name) {
    ...
    if (\Facebook\Libphutil\Functions\utils::idx($this->results, $name) !== null) {
...
/* -(  Command Help  )------------------------------------------------------- */
  ...
  public function renderWorkflowHelp(
    ...
    $workflow = \Facebook\Libphutil\Functions\utils::idx($this->workflows, strtolower($workflow_name));
    if (!$workflow) {
      $out[] = $this->indent(
        $indent,
  ...
  public function printUsageException(\Facebook\Libphutil\PhutilArgumentUsageException $ex) {
    fwrite(
      STDERR,
...
/* -(  Internals  )---------------------------------------------------------- */
  ...
  private function mergeSpecs(array $specs) {
    ...
    $short_map = \Facebook\Libphutil\Functions\utils::mpull($this->specs, null, 'getShortAlias');
  ...
  private function renderArgumentSpecs(array $specs) {
    ...
    $specs = \Facebook\Libphutil\Functions\utils::msort($specs, 'getName');
    foreach ($specs as $spec) {
  ...
  private function format($str /* , ... */) {
    ...
    return call_user_func_array(
      '\Facebook\Libphutil\Functions\format::phutil_console_format',
      $args);
  }
  ...
  private function indent($level, $str /* , ... */) {
    ...
    return \Facebook\Libphutil\Functions\format::phutil_console_wrap($text, $level);
  }
  ...
  public function shutdownProfiler() {
    $data = xhprof_disable();
    $data = serialize($data);
    \Facebook\Libphutil\Filesystem::writeFile($this->getArg('xprofile'), $data);
  }
