 ...
 *
 * \Facebook\Libphutil\Filesystem will resolve relative paths against PWD from the environment.
 * When \Facebook\Libphutil\Filesystem is unable to complete an operation, it throws a
 * FilesystemException.
 *
...
/* -(  Files  )-------------------------------------------------------------- */
  ...
  public static function writeUniqueFile($base, $data) {
    $full_path = \Facebook\Libphutil\Filesystem::resolvePath($base);
    $sequence = 0;
    \Facebook\Libphutil\Functions\utils::assert_stringlike($data);
    ...
    while (true) {
      ...
      if ($handle) {
        ...
        if ($ok === false) {
          throw new \Facebook\Libphutil\FilesystemException(
            $try_path,
            \Facebook\Libphutil\Functions\pht::pht("Failed to write file data."));
        }
        ...
        if (!$ok) {
          throw new \Facebook\Libphutil\FilesystemException(
            $try_path,
            \Facebook\Libphutil\Functions\pht::pht("Failed to close file handle."));
        }
  ...
  private static function executeRemovePath($path) {
    if (is_dir($path) && !is_link($path)) {
      foreach (\Facebook\Libphutil\Filesystem::listDirectory($path, true) as $child) {
  ...
  public static function readRandomBytes($number_of_bytes) {
    ...
    if ($number_of_bytes < 1) {
      throw new \Exception(\Facebook\Libphutil\Functions\pht::pht("You must generate at least 1 byte of entropy."));
    }
    ...
    if (function_exists('openssl_random_pseudo_bytes')) {
      ...
      if ($data === false) {
        throw new \Exception(
          \Facebook\Libphutil\Functions\pht::pht('openssl_random_pseudo_bytes() failed to generate entropy!'));
      }
    ...
    if (\Facebook\Libphutil\Functions\utils::phutil_is_windows()) {
      throw new \Exception(
        \Facebook\Libphutil\Functions\pht::pht(
          '\Facebook\Libphutil\Filesystem::readRandomBytes() requires the PHP OpenSSL extension '.
  ...
  public static function getMimeType(
    ...
    if ($mime_type === null) {
      list($err, $stdout) = \Facebook\Libphutil\Functions\execx::exec_manual(
        'file --brief --mime %s',
        $path);
      if (!$err) {
...
/* -(  Directories  )-------------------------------------------------------- */
  ...
  public static function createDirectory($path, $umask = 0777,
    ...
    if (is_dir($path)) {
      \Facebook\Libphutil\Filesystem::changePermissions($path, $umask);
      return $path;
    }
   ...
   * responsible for removing it (e.g., with \Facebook\Libphutil\Filesystem::remove())
  ...
  public static function createTemporaryDirectory($prefix = '', $umask = 0700) {
    ...
    if (!$tries) {
      ...
      if ($df !== false && $df < 1024 * 1024) {
        throw new \Facebook\Libphutil\FilesystemException(
          $dir,
          \Facebook\Libphutil\Functions\pht::pht("Failed to create a temporary directory: the disk is full."));
      }

      throw new \Facebook\Libphutil\FilesystemException(
        $dir,
        \Facebook\Libphutil\Functions\pht::pht("Failed to create a temporary directory in '%s'.", $tmp));
    }
...
/* -(  Paths  )-------------------------------------------------------------- */
  ...
  public static function isDescendant($path, $root) {
    ...
    } catch (\Facebook\Libphutil\FilesystemException $e) {
      return false;
    }
    $fs = new \Facebook\Libphutil\FileList(array($root));
  ...
  public static function resolveBinary($binary) {
    if (\Facebook\Libphutil\Functions\utils::phutil_is_windows()) {
      list($err, $stdout) = \Facebook\Libphutil\Functions\execx::exec_manual('where %s', $binary);
      $stdout = \Facebook\Libphutil\Functions\utils::phutil_split_lines($stdout);
      if (!$stdout) {
        return null;
      }
      $stdout = \Facebook\Libphutil\Functions\utils::head($stdout);
    } else {
      list($err, $stdout) = \Facebook\Libphutil\Functions\execx::exec_manual('which %s', $binary);
    }
  ...
  public static function pathsAreEquivalent($u, $v) {
    $u = \Facebook\Libphutil\Filesystem::resolvePath($u);
    $v = \Facebook\Libphutil\Filesystem::resolvePath($v);
...
/* -(  Assert  )------------------------------------------------------------- */
  ...
  public static function assertExists($path) {
    if (!self::pathExists($path)) {
      throw new \Facebook\Libphutil\FilesystemException(
        $path,
        "\Facebook\Libphutil\Filesystem entity `{$path}' does not exist.");
    }
  }
