...
class PhutilTranslator {
  ...
  public static function getInstance() {
    if (self::$instance === null) {
      self::$instance = new \Facebook\Libphutil\PhutilTranslator();
    }
    return self::$instance;
  }

  public static function setInstance(\Facebook\Libphutil\PhutilTranslator $instance) {
   ...
   * @return \Facebook\Libphutil\PhutilTranslator Provides fluent interface.
   */
  ...
  public function translate($text /* , ... */) {
    $translation = \Facebook\Libphutil\Functions\utils::idx($this->translations, $text, $text);
    $args = func_get_args();
    ...
    foreach ($args as $k => $arg) {
      if ($arg instanceof \Facebook\Libphutil\PhutilNumber) {
    ...
    $is_html = false;
    foreach ($args as $arg) {
      if ($arg instanceof \Facebook\Libphutil\PhutilSafeHTML) {
        $is_html = true;
        break;
      }
    }
    ...
    if ($is_html) {
      $result = \Facebook\Libphutil\Functions\render::phutil_safe_html($result);
    }
  ...
  private function chooseVariant(array $translations, $variant) {
    ...
    if ($variant instanceof \Facebook\Libphutil\PhutilNumber) {
    ...
    switch ($this->language) {
      ...
      case 'cs':
        if ($variant instanceof \Facebook\Libphutil\PhutilPerson) {
          list($male, $female) = $translations;
          if ($variant->getSex() == \Facebook\Libphutil\PhutilPerson::SEX_FEMALE) {
            return $female;
          }
          return $male;
        }
