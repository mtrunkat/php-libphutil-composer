...
class PhutilRemarkupRuleHyperlink
  ...
  public function apply($text) {
    ...
    $text = preg_replace_callback(
      '@<(\w{3,}://[^\s'.\Facebook\Libphutil\PhutilRemarkupBlockStorage::MAGIC_BYTE.']+?)>@',
  ...
  protected function markupHyperlink($matches) {
    ...
    $protocol = \Facebook\Libphutil\Functions\utils::id(new \Facebook\Libphutil\PhutilURI($matches[1]))->getProtocol();
    if (!\Facebook\Libphutil\Functions\utils::idx($protocols, $protocol)) {
  ...
  protected function markupHyperlinkUngreedy($matches) {
    ...
    return \Facebook\Libphutil\Functions\render::hsprintf('%s%s', $this->markupHyperlink(array(null, $match)), $tail);
  }
