...
final class PhutilServiceProfiler {
  ...
  public static function getInstance() {
    if (empty(self::$instance)) {
      self::$instance = new \Facebook\Libphutil\PhutilServiceProfiler();
    }
  ...
  public static function installEchoListener() {
    $instance = \Facebook\Libphutil\PhutilServiceProfiler::getInstance();
    $instance->addListener(array('\Facebook\Libphutil\PhutilServiceProfiler', 'echoListener'));
  }
  ...
  public static function echoListener($type, $id, $data) {
    ...
    $type = \Facebook\Libphutil\Functions\utils::idx($data, 'type', 'mystery');
    ...
    if ($is_begin) {
      switch ($type) {
        ...
        case 'conduit':
          ...
          break;
        case 'http':
          $desc = \Facebook\Libphutil\Functions\utils::phutil_censor_credentials($data['uri']);
          break;
        case 'lint':
          $desc = $data['linter'];
          if (isset($data['paths'])) {
        ...
        case 'event':
          ...
          break;
        case 'ldap':
          $call = \Facebook\Libphutil\Functions\utils::idx($data, 'call', '?');
          $params = array();
          switch ($call) {
            case 'connect':
    ...
    $console = \Facebook\Libphutil\PhutilConsole::getConsole();
    $console->writeLog(
      "%s [%s] <%s> %s\n",
      $mark,
      $id,
