...
final class PhutilXHPASTSyntaxHighlighterFuture extends \Facebook\Libphutil\FutureProxy {
  ...
  private function applyXHPHighlight($result) {
    ...
    $tree = \Facebook\Libphutil\XHPASTTree::newFromDataAndResolvedExecFuture(
      $this->source,
      $result);
    ...
    foreach ($tokens as $key => $token) {
      ...
      if (isset($interesting_symbols[$key])) {
        ...
        $class = $sym[0];
        $attrs['data-symbol-context'] = \Facebook\Libphutil\Functions\utils::idx($sym, 'context');
        $attrs['data-symbol-name'] = \Facebook\Libphutil\Functions\utils::idx($sym, 'symbol');
      } else {
      ...
      if ($class) {
        ...
        if ($multi) {
          ...
          $value = \Facebook\Libphutil\Functions\utils::phutil_split_lines($value, $retain_endings = true);
        } else {
          $value = array($value);
        }
        foreach ($value as $val) {
          $out[] = \Facebook\Libphutil\Functions\render::phutil_tag('span', $attrs, $val);
        }
      } else {
        $out[] = $value;
      }
  ...
  private function findInterestingSymbols(\Facebook\Libphutil\XHPASTNode $root) {
    ...
    foreach ($static_access as $access) {
      ...
      if ($class->getTypeName() == 'n_CLASS_NAME' &&
        ...
        $classname = \Facebook\Libphutil\Functions\utils::head($class->getTokens())->getValue();
        $result = array(
          'na',
