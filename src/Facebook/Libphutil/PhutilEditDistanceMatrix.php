 ...
 *
 *  $matrix = \Facebook\Libphutil\Functions\utils::id(new \Facebook\Libphutil\PhutilEditDistanceMatrix())
...
final class PhutilEditDistanceMatrix {
  ...
  public function getMaximumLength() {
    return \Facebook\Libphutil\Functions\utils::coalesce($this->maximumLength, $this->getInfinity());
  }
  ...
  private function printMatrix(array $m) {
    ...
    $p = '% 8s ';
    printf($p, ' ');
    foreach (\Facebook\Libphutil\Functions\utils::head($m) as $yk => $yv) {
      printf($p, \Facebook\Libphutil\Functions\utils::idx($y, $yk - 1, '-'));
    }
    echo "\n";
    foreach ($m as $xk => $xv) {
      printf($p, \Facebook\Libphutil\Functions\utils::idx($x, $xk - 1, '-'));
      foreach ($xv as $yk => $yv) {
  ...
  private function printTypeMatrix(array $t) {
    ...
    $p = '% 8s ';
    printf($p, ' ');
    foreach (\Facebook\Libphutil\Functions\utils::head($t) as $yk => $yv) {
      printf($p, \Facebook\Libphutil\Functions\utils::idx($y, $yk - 1, '-'));
    }
    echo "\n";
    foreach ($t as $xk => $xv) {
