 ...
 *
 *    \Facebook\Libphutil\AphrontWriteGuard::willWrite();
 *
 ...
 *
 *    \Facebook\Libphutil\AphrontWriteGuard::beginUnguardedWrites();
 *    do_logging_write();
...
/* -(  Managing Write Guards  )---------------------------------------------- */
   ...
   *
   *    $guard = new \Facebook\Libphutil\AphrontWriteGuard($callback);
   *    // ...
   *    $guard->dispose();
   *
  ...
  public function __construct($callback) {
    if (self::$instance) {
      throw new \Exception(
        "An \Facebook\Libphutil\AphrontWriteGuard already exists. Dispose of the previous guard ".
        "before creating a new one.");
    }
    if (self::$allowUnguardedWrites) {
      throw new \Exception(
        "An \Facebook\Libphutil\AphrontWriteGuard is being created in a context which permits ".
    ...
    if (!self::$abruptExitlistenerIsInstalled) {
      ...
      $event_listener = new \Facebook\Libphutil\AphrontWriteGuardExitEventListener();
  ...
  /**
   * Return on instance of \Facebook\Libphutil\AphrontWriteGuard if it's active, or null
   *
...
/* -(  Protecting Writes  )-------------------------------------------------- */
  ...
  public static function willWrite() {
    if (!self::$instance) {
      if (!self::$allowUnguardedWrites) {
        throw new \Exception(
          "Unguarded write! There must be an active \Facebook\Libphutil\AphrontWriteGuard to ".
          "perform writes.");
      } else {
...
/* -(  Disabling Write Protection  )----------------------------------------- */
   ...
   *     $unguarded = \Facebook\Libphutil\AphrontWriteGuard::beginScopedUnguardedWrites();
   *     do_logging();
   *   }
   *
   * @return \Facebook\Libphutil\AphrontScopedUnguardedWriteCapability Object which ends unguarded
   *            writes when it leaves scope.
   * @task disable
   */
  public static function beginScopedUnguardedWrites() {
    self::beginUnguardedWrites();
    return new \Facebook\Libphutil\AphrontScopedUnguardedWriteCapability();
  }
  ...
  public static function allowDangerousUnguardedWrites($allow) {
    if (self::$instance) {
      throw new \Exception(
        "You can not unconditionally disable \Facebook\Libphutil\AphrontWriteGuard by calling ".
...
/* -(  Internals  )---------------------------------------------------------- */
  ...
  public function __destruct() {
    if (isset(self::$instance)) {
      throw new \Exception(
        "\Facebook\Libphutil\AphrontWriteGuard was not properly disposed of! Call dispose() on ".
        "every \Facebook\Libphutil\AphrontWriteGuard object you instantiate or use \Facebook\Libphutil\Functions\utils::phutil_exit() ".
