...
abstract class AphrontDatabaseConnection
  implements \Facebook\Libphutil\PhutilQsprintfInterface {
  ...
  public function queryData($pattern/* , $arg, $arg, ... */) {
    ...
    return call_user_func_array('\Facebook\Libphutil\Functions\queryfx::queryfx_all', $args);
  }

  public function query($pattern/* , $arg, $arg, ... */) {
    $args = func_get_args();
    array_unshift($args, $this);
    return call_user_func_array('\Facebook\Libphutil\Functions\queryfx::queryfx', $args);
  }
...
/* -(  Transaction Management  )--------------------------------------------- */
   ...
   * one if none exists.
   *
   * @return \Facebook\Libphutil\AphrontDatabaseTransactionState Current transaction state.
   * @task xaction
   */
  protected function getTransactionState() {
    if (!$this->transactionState) {
      $this->transactionState = new \Facebook\Libphutil\AphrontDatabaseTransactionState();
    }
