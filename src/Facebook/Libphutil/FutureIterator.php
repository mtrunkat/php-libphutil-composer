...
/**
 * \Facebook\Libphutil\FutureIterator aggregates @{class:Future}s and allows you to respond to them
 ...
 *
 *   $futures = array(
 *     'a.txt' => new \Facebook\Libphutil\ExecFuture('wc -c a.txt'),
 *     'b.txt' => new \Facebook\Libphutil\ExecFuture('wc -c b.txt'),
 *     'c.txt' => new \Facebook\Libphutil\ExecFuture('wc -c c.txt'),
 *   );
 *
 *   foreach (\Facebook\Libphutil\Functions\functions::Futures($futures) as $key => $future) {
...
/* -(  Basics  )------------------------------------------------------------- */
  ...
  public function __construct(array $futures) {
    \Facebook\Libphutil\Functions\utils::assert_instances_of($futures, '\Facebook\Libphutil\Future');
   ...
   * others.
   *
   * @param \Facebook\Libphutil\Future  @{class:Future} to add to iterator
   * @task basics
   */
  public function addFuture(\Facebook\Libphutil\Future $future, $key = null) {
    if ($key === null) {
      $this->futures[] = $future;
      $this->wait[] = \Facebook\Libphutil\Functions\utils::last_key($this->futures);
...
/* -(  Configuring Iteration  )---------------------------------------------- */
   ...
   *
   *   foreach (\Facebook\Libphutil\Functions\functions::Futures($futures)->setUpdateInterval(1) as $future) {
   *     if ($future === null) {
   *       echo "Still working...\n";
   *     } else {
   *       // ...
   ...
   * resolving. By default, \Facebook\Libphutil\FutureIterator never yields null.
   *
   ...
   *
   *  foreach (\Facebook\Libphutil\Functions\functions::Futures($futures)->limit(4) as $future) {
  ...
  public function next() {
    ...
    do {
      ...
      if ($resolve === null) {
        ...
        if ($can_use_sockets) {
          \Facebook\Libphutil\Future::waitForSockets($read_sockets, $write_sockets, $wait_time);
        } else {
          usleep(1000);
        }
      }
