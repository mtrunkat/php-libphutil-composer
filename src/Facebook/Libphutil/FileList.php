 ...
 *
 *    $list = new \Facebook\Libphutil\FileList(array_slice($argv, 1));
...
final class FileList {
  ...
  /**
   * Build a new \Facebook\Libphutil\FileList from an array of paths, e.g. from $argv.
   *
  ...
  public function __construct($paths) {
    foreach ($paths as $path) {
      $path = \Facebook\Libphutil\Filesystem::resolvePath($path);
      if (is_dir($path)) {
