...
/* -(  Lock Registry  )------------------------------------------------------ */
  ...
  protected static function getLock($name) {
    return \Facebook\Libphutil\Functions\utils::idx(self::$locks, $name);
  }
   ...
   *
   * @param \Facebook\Libphutil\PhutilLock Lock to register.
   * @task registry
   */
  protected static function registerLock(\Facebook\Libphutil\PhutilLock $lock) {
    if (!self::$registeredShutdownFunction) {
      register_shutdown_function(array('\Facebook\Libphutil\PhutilLock', 'unlockAll'));
