 ...
 * @group markup
 */
class PhutilRemarkupEngineRemarkupTableBlockRule
  ...
  private function fail($near, $message) {
    $message = sprintf(
      '%s near: %s',
      $message,
      \Facebook\Libphutil\Functions\utf8::phutil_utf8_shorten($near, 32000));

    if ($this->getEngine()->isTextMode()) {
      return '('.$message.')';
    }

    return \Facebook\Libphutil\Functions\render::hsprintf('<div style="color: red;">%s</div>', $message);
  }
