...
class utf8 {
   ...
   * @group utf8
   */
  static function phutil_utf8ize($string) {
    if (\Facebook\Libphutil\Functions\utf8::phutil_is_utf8($string)) {
      return $string;
    }
   ...
   */
  static function phutil_is_utf8_with_only_bmp_characters($string) {
   ...
   * @group utf8
   */
  static function phutil_is_utf8($string) {
   ...
   * @group utf8
   */
  static function phutil_utf8_strlen($string) {
   ...
   * @group   utf8
   */
  static function phutil_utf8_console_strlen($string) {
    $string_v = \Facebook\Libphutil\Functions\utf8::phutil_utf8v_codepoints($string);
   ...
   * @group utf8
   */
  static function phutil_utf8v($string) {
    $res = array();
    $len = strlen($string);
    $ii = 0;
    while ($ii < $len) {
      ...
      if ($byte <= "\x7F") {
        ...
        throw new \Exception("Invalid UTF-8 string passed to \Facebook\Libphutil\Functions\utf8::phutil_utf8v().");
      } else if ($byte <= "\xDF") {
        $seq_len = 2;
      } else if ($byte <= "\xEF") {
        $seq_len = 3;
   ...
   * @group   utf8
   */
  static function phutil_utf8v_codepoints($string) {
    $str_v = \Facebook\Libphutil\Functions\utf8::phutil_utf8v($string);
   ...
   * @group utf8
   */
  static function phutil_utf8_shorten($string, $length, $terminal = "\xE2\x80\xA6") {
    ...
    $string_v = \Facebook\Libphutil\Functions\utf8::phutil_utf8v_combined($string);
    ...
    $terminal_len = \Facebook\Libphutil\Functions\utf8::phutil_utf8_strlen($terminal);
   ...
   * @group utf8
   */
  static function phutil_utf8_hard_wrap_html($string, $width) {
    $break_here = array();
  
    // Convert the UTF-8 string into a list of UTF-8 characters.
    $vector = \Facebook\Libphutil\Functions\utf8::phutil_utf8v($string);
    $len = count($vector);
    $char_pos = 0;
    for ($ii = 0; $ii < $len; ++$ii) {
  ...
  /**
    ...
    * @group utf8
    */
  static function phutil_utf8_hard_wrap($string, $width) {
    $result = array();
  
    $lines = \Facebook\Libphutil\Functions\utils::phutil_split_lines($string, $retain_endings = false);
    foreach ($lines as $line) {
  
      // Convert the UTF-8 string into a list of UTF-8 characters.
      $vector = \Facebook\Libphutil\Functions\utf8::phutil_utf8v($line);
   ...
   */
  static function phutil_utf8_convert($string, $to_encoding, $from_encoding) {
    if (!$from_encoding) {
    ...
    if ($result === false) {
      ...
      if ($message) {
        $message = \Facebook\Libphutil\Functions\utils::idx($message, 'message', 'Unknown error.');
      }
      throw new \Exception(
   ...
   * @group utf8
   */
  static function phutil_utf8_ucwords($str) {
    ...
    $v = \Facebook\Libphutil\Functions\utf8::phutil_utf8v($str);
    $result = '';
    $last = null;
    ...
    foreach ($v as $c) {
      ...
      if ($convert) {
        $result .= \Facebook\Libphutil\Functions\utf8::phutil_utf8_strtoupper($c);
      } else {
        $result .= $c;
      }
   ...
   */
  static function phutil_utf8_strtolower($str) {
    ...
    return \Facebook\Libphutil\Functions\utf8::phutil_utf8_strtr($str, $map);
  }
   ...
   * @phutil-external-symbol function mb_convert_case
   */
  static function phutil_utf8_strtoupper($str) {
    if (function_exists('mb_convert_case')) {
   ...
   * @group utf8
   */
  static function phutil_utf8_strtr($str, array $map) {
    $v = \Facebook\Libphutil\Functions\utf8::phutil_utf8v($str);
    $result = '';
    foreach ($v as $c) {
      if (isset($map[$c])) {
  ...
  static function phutil_utf8_is_combining_character($character) {
    $components = \Facebook\Libphutil\Functions\utf8::phutil_utf8v_codepoints($character);
  ...
  static function phutil_utf8v_combined($string) {
    $components = \Facebook\Libphutil\Functions\utf8::phutil_utf8v($string);
    ...
    if (
      $array_length > 0 &&
      \Facebook\Libphutil\Functions\utf8::phutil_utf8_is_combining_character($components[0])) {
      $string = " ".$string;
      $components = \Facebook\Libphutil\Functions\utf8::phutil_utf8v($string);
      $array_length++;
    }
    ...
    for ($index = 1; $index < $array_length; $index++) {
      if (\Facebook\Libphutil\Functions\utf8::phutil_utf8_is_combining_character($components[$index])) {
        $components[$index - 1] =
