...
class jsprintf {
   ...
   * @group markup
   */
  static function jsprintf($pattern /* , ... */) {
    $args = func_get_args();
    return \Facebook\Libphutil\Functions\xsprintf::xsprintf('\Facebook\Libphutil\Functions\jsprintf::xsprintf_javascript', null, $args);
  }
   ...
   * @group markup
   */
  static function vjsprintf($pattern, array $argv) {
    array_unshift($argv, $pattern);
    return \Facebook\Libphutil\Functions\xsprintf::xsprintf('\Facebook\Libphutil\Functions\jsprintf::xsprintf_javascript', null, $argv);
  }
  
  
  /**
   * \Facebook\Libphutil\Functions\xsprintf::xsprintf() callback for javascript encoding.
   * @group markup
   */
  static function xsprintf_javascript($userdata, &$pattern, &$pos, &$value, &$length) {
    ...
    switch ($type) {
      ...
      case 'd':
        if ($value > 0x1FFFFFFFFFFFFF) {
          throw new \Exception(
            "You are passing an integer to \Facebook\Libphutil\Functions\jsprintf::jsprintf() which is so large it can ".
