...
class xsprintf {
   ...
   * in the string. For instance, if you call \Facebook\Libphutil\Functions\xsprintf::xsprintf() like this...
   *
   *    $result = \Facebook\Libphutil\Functions\xsprintf::xsprintf(
   *      '\Facebook\Libphutil\Functions\xsprintf::xsprintf_callback_example',
   *      $userdata = null,
   *      array(
   *        "The %M is made of %C.",
   *        'moon',
   ...
   *
   */
  static function xsprintf($callback, $userdata, $argv) {
    $argc = count($argv);
    $arg  = 0;
    $pos  = 0;
    $pattern = $argv[0];
    ...
    for ($pos = 0; $pos < $len; $pos++) {
      ...
      if ($conv) {
        ...
        if (strpos("'-0123456789.\$+", $c) !== false) {
          throw new \Exception(
            "\Facebook\Libphutil\Functions\xsprintf::xsprintf() does not support the `%{$c}' modifier.");
        }
        ...
        if ($c != '%') {
          ...
          $arg++;
          if ($arg >= $argc) {
            throw new \Exception("Too few arguments to \Facebook\Libphutil\Functions\xsprintf::xsprintf().");
          }
    ...
    if ($arg != ($argc - 1)) {
      throw new \Exception("Too many arguments to \Facebook\Libphutil\Functions\xsprintf::xsprintf().");
    }
  ...
  /**
   * Example @{function:xsprintf} callback. When you call \Facebook\Libphutil\Functions\xsprintf::xsprintf(), you
   * must pass a callback like this one. \Facebook\Libphutil\Functions\xsprintf::xsprintf() will invoke the callback when
   ...
   */
  static function xsprintf_callback_example(
    $userdata,
    &$pattern,
    &$pos,
    &$value,
    &$length) {
    throw new \Exception(
      "This function exists only to document the call signature for \Facebook\Libphutil\Functions\xsprintf::xsprintf() ".
      "callbacks.");
  }
