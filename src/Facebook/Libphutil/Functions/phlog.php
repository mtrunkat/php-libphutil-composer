...
class phlog {
    ...
    foreach (func_get_args() as $event) {
      \Facebook\Libphutil\PhutilErrorHandler::dispatchErrorMessage(
        $event instanceof \Exception
          ? \Facebook\Libphutil\PhutilErrorHandler::EXCEPTION
          : \Facebook\Libphutil\PhutilErrorHandler::PHLOG,
        $event,
        $metadata);
    }
   ...
   *
   * @param const A \Facebook\Libphutil\PhutilErrorHandler constant, like \Facebook\Libphutil\PhutilErrorHandler::ERROR,
   ...
   * @group error
   */
  static function phutil_error_listener_example($event, $value, array $metadata) {
