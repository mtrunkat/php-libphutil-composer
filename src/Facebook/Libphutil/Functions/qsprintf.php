...
class qsprintf {
   ...
   *
   *       \Facebook\Libphutil\Functions\qsprintf::qsprintf($escaper, 'WHERE hatID %=d', $hat);
   *
   *   %Ld, %Ls, %Lf, %LB
   *     "List" versions of %d, %s, %f and %B. These are appropriate for use in
   *     an "IN" clause. For example:
   *
   *       \Facebook\Libphutil\Functions\qsprintf::qsprintf($escaper, 'WHERE hatID IN(%Ld)', $list_of_hats);
   *
   *   %B ("Binary String")
   ...
   *       \Facebook\Libphutil\Functions\qsprintf::qsprintf($escaper, 'WHERE name LIKE %~', $search);
   *
   *     See also %> and %<.
   *
   *   %> ("Prefix")
   ...
   *       \Facebook\Libphutil\Functions\qsprintf::qsprintf($escaper, 'WHERE name LIKE %>', $prefix);
   *
   *   %< ("Suffix")
   *     Escapes a suffix query for a LIKE clause. For example:
   *
   *       //  Find all rows where `name` ends with $suffix.
   *       \Facebook\Libphutil\Functions\qsprintf::qsprintf($escaper, 'WHERE name LIKE %<', $suffix);
   *
   * @group storage
   */
  static function qsprintf(\Facebook\Libphutil\PhutilQsprintfInterface $escaper, $pattern/* , ... */) {
    $args = func_get_args();
    array_shift($args);
    return \Facebook\Libphutil\Functions\xsprintf::xsprintf('\Facebook\Libphutil\Functions\qsprintf::xsprintf_query', $escaper, $args);
  }
  
  /**
   * @group storage
   */
  static function vqsprintf(\Facebook\Libphutil\PhutilQsprintfInterface $escaper, $pattern, array $argv) {
    array_unshift($argv, $pattern);
    return \Facebook\Libphutil\Functions\xsprintf::xsprintf('\Facebook\Libphutil\Functions\qsprintf::xsprintf_query', $escaper, $argv);
  }
  
  
  /**
   * \Facebook\Libphutil\Functions\xsprintf::xsprintf() callback for encoding SQL queries. See \Facebook\Libphutil\Functions\qsprintf::qsprintf().
   * @group storage
   */
  static function xsprintf_query($userdata, &$pattern, &$pos, &$value, &$length) {
    $type    = $pattern[$pos];
    $escaper = $userdata;
    ...
    if (!($escaper instanceof \Facebook\Libphutil\PhutilQsprintfInterface)) {
    ...
    switch ($type) {
      ...
      case 'L': // List of..
        \Facebook\Libphutil\Functions\qsprintf::_qsprintf_check_type($value, "L{$next}", $pattern);
        $pattern = substr_replace($pattern, '', $pos, 1);
        $length  = strlen($pattern);
        $type = 's';
        $done = true;
    ...
    if (!$done) {
      \Facebook\Libphutil\Functions\qsprintf::_qsprintf_check_type($value, $type, $pattern);
      switch ($type) {
        case 's': // String
   ...
   * @group storage
   */
  static function _qsprintf_check_type($value, $type, $query) {
    switch ($type) {
      case 'Ld': case 'Ls': case 'LC': case 'LB':
        if (!is_array($value)) {
          throw new \Facebook\Libphutil\AphrontQueryParameterException(
            $query,
        ...
        foreach ($value as $scalar) {
          \Facebook\Libphutil\Functions\qsprintf::_qsprintf_check_scalar_type($scalar, $type, $query);
        }
        break;
      default:
        \Facebook\Libphutil\Functions\qsprintf::_qsprintf_check_scalar_type($value, $type, $query);
        break;
    }
  }
   ...
   * @group storage
   */
  static function _qsprintf_check_scalar_type($value, $type, $query) {
    switch ($type) {
