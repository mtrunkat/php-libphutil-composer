...
class utils {
   ...
   * ...but this works fine:
   *
   *    \Facebook\Libphutil\Functions\utils::id(new Thing())->doStuff();
   *
   * @param   wild Anything.
   * @return  wild Unmodified argument.
   */
  static function id($x) {
    return $x;
  }
   ...
   */
  static function idx(array $array, $key, $default = null) {
   ...
   *    }
   *
   * You can express this more concisely with \Facebook\Libphutil\Functions\utils::mpull():
   *
   *    $names = \Facebook\Libphutil\Functions\utils::mpull($objects, 'getName');
   *
   * \Facebook\Libphutil\Functions\utils::mpull() takes a third argument, which allows you to do the same but for
   * the array's keys:
   *
   *    COUNTEREXAMPLE
   *    $names = array();
   ...
   *
   *    $names = \Facebook\Libphutil\Functions\utils::mpull($objects, 'getName', 'getID');
   *
   ...
   *    }
   *
   * With \Facebook\Libphutil\Functions\utils::mpull():
   *
   *    $id_map = \Facebook\Libphutil\Functions\utils::mpull($objects, null, 'getID');
   *
   ...
   */
  static function mpull(array $list, $method, $key_method = null) {
    $result = array();
   ...
   *    }
   *
   * You can express this more concisely with \Facebook\Libphutil\Functions\utils::ppull():
   *
   *    $names = \Facebook\Libphutil\Functions\utils::ppull($objects, 'name');
   *
   * \Facebook\Libphutil\Functions\utils::ppull() takes a third argument, which allows you to do the same but for
   * the array's keys:
   *
   *    COUNTEREXAMPLE
   *    $names = array();
   ...
   *
   *    $names = \Facebook\Libphutil\Functions\utils::ppull($objects, 'name', '\Facebook\Libphutil\Functions\utils::id');
   *
   * If you pass ##null## as the second argument, the objects will be preserved:
   *
   *    COUNTEREXAMPLE
   ...
   *    }
   *
   * With \Facebook\Libphutil\Functions\utils::ppull():
   *
   *    $id_map = \Facebook\Libphutil\Functions\utils::ppull($objects, null, '\Facebook\Libphutil\Functions\utils::id');
   *
   ...
   *                        ##null## to preserve the original objects.
   * @param   string|null   Determines how **keys** will be assigned in the result
   *                        array. Use a string like '\Facebook\Libphutil\Functions\utils::id' to use the result of
   *                        accessing the named property as each object's key, or
   *                        ##null## to preserve the original keys.
   * @return  dict          A dictionary with keys and values derived according
   *                        to whatever you passed as $property and $key_property.
   */
  static function ppull(array $list, $property, $key_property = null) {
    $result = array();
    foreach ($list as $key => $object) {
   ...
   *    }
   *
   * With \Facebook\Libphutil\Functions\utils::ipull():
   *
   *    $names = \Facebook\Libphutil\Functions\utils::ipull($list, 'name');
   *
   ...
   */
  static function ipull(array $list, $index, $key_index = null) {
    $result = array();
   ...
   *    }
   *
   * This can be expressed more tersely with \Facebook\Libphutil\Functions\utils::mgroup():
   *
   *    $animals_by_species = \Facebook\Libphutil\Functions\utils::mgroup($animals, 'getSpecies');
   *
   ...
   */
  static function mgroup(array $list, $by /* , ... */) {
    $map = \Facebook\Libphutil\Functions\utils::mpull($list, $by);
    ...
    if ($args) {
      ...
      foreach ($groups as $group_key => $grouped) {
        $args[0] = $grouped;
        $groups[$group_key] = call_user_func_array('\Facebook\Libphutil\Functions\utils::mgroup', $args);
      }
    }
   ...
   */
  static function igroup(array $list, $by /* , ... */) {
    $map = \Facebook\Libphutil\Functions\utils::ipull($list, $by);
    ...
    if ($args) {
      ...
      foreach ($groups as $group_key => $grouped) {
        $args[0] = $grouped;
        $groups[$group_key] = call_user_func_array('\Facebook\Libphutil\Functions\utils::igroup', $args);
      }
    }
   ...
   *    $sorted = \Facebook\Libphutil\Functions\utils::msort($ducks, 'getName');
   *
   ...
   */
  static function msort(array $list, $method) {
    $surrogate = \Facebook\Libphutil\Functions\utils::mpull($list, $method);
   ...
   */
  static function isort(array $list, $index) {
    $surrogate = \Facebook\Libphutil\Functions\utils::ipull($list, $index);
   ...
   *
   *   \Facebook\Libphutil\Functions\utils::mfilter($list, 'hasChildren');
   *
   ...
   *
   *   \Facebook\Libphutil\Functions\utils::mfilter($list, 'hasChildren', true);
   *
   ...
   */
  static function mfilter(array $list, $method, $negate = false) {
    if (!is_string($method)) {
   ...
   *
   *   \Facebook\Libphutil\Functions\utils::ifilter($list, 'username');
   *
   * The optional third parameter allows you to negate the operation and filter
   * out nonempty arrays. To remove all arrays that DO have value for key
   * 'username', do this:
   *
   *   \Facebook\Libphutil\Functions\utils::ifilter($list, 'username', true);
   *
   ...
   */
  static function ifilter(array $list, $index, $negate = false) {
    if (!is_scalar($index)) {
   ...
   */
  static function array_select_keys(array $dict, array $keys) {
    $result = array();
    foreach ($keys as $key) {
   ...
   */
  static function assert_instances_of(array $arr, $class) {
   ...
   * @task   assert
   */
  static function assert_stringlike($parameter) {
   ...
   *
   *   $display_name = \Facebook\Libphutil\Functions\utils::nonempty($user_name, $full_name, "Anonymous");
   *
   * @param  ...         Zero or more arguments of any type.
   ...
   * function:
   *
   *   $obj = \Facebook\Libphutil\Functions\utils::newv($class_name, $argv);
   *
   * That is, these two statements are equivalent:
   *
   *   $pancake = new Pancake('Blueberry', 'Maple Syrup', true);
   *   $pancake = \Facebook\Libphutil\Functions\utils::newv('Pancake', array('Blueberry', 'Maple Syrup', true));
   *
   ...
   * construction methods) might make it cleaner before using \Facebook\Libphutil\Functions\utils::newv(). Static
   ...
   */
  static function newv($class_name, array $argv) {
   ...
   */
  static function head(array $arr) {
    return reset($arr);
  }
   ...
   */
  static function head_key(array $arr) {
    reset($arr);
    return key($arr);
  }
   ...
   *
   *   array_merge($a, $b, $c);
   *   \Facebook\Libphutil\Functions\utils::array_mergev(array($a, $b, $c));
   *
   ...
   */
  static function array_mergev(array $arrayv) {
    if (!$arrayv) {
      return array();
    }
   ...
   */
  static function phutil_split_lines($corpus, $retain_endings = true) {
    if (!strlen($corpus)) {
      return array('');
    }
    ...
    if ($corpus instanceof \Facebook\Libphutil\PhutilSafeHTML) {
      return array_map('\Facebook\Libphutil\Functions\render::phutil_safe_html', $lines);
    }
   ...
   *
   *   $result = \Facebook\Libphutil\Functions\utils::array_fuse($list);
   *
   * @param   list  List of scalars.
   * @return  dict  Dictionary with inputs mapped to themselves.
   */
  static function array_fuse(array $list) {
    if ($list) {
   ...
   *
   *   implode('', \Facebook\Libphutil\Functions\utils::array_interleave($x, $list));
   *
   * ...is equivalent to:
   *
   *   implode($x, $list);
   ...
   */
  static function array_interleave($interleave, array $array) {
    $result = array();
   ...
   * @group library
   */
  static function phutil_is_windows() {
   ...
   * @group library
   */
  static function phutil_is_hiphop_runtime() {
   ...
   * @group library
   */
  static function phutil_exit($status = 0) {
    $event = new \Facebook\Libphutil\PhutilEvent(
      \Facebook\Libphutil\PhutilEventType::TYPE_WILLEXITABRUPTLY,
      array("status" => $status));
    \Facebook\Libphutil\PhutilEventEngine::dispatchEvent($event);
   ...
   */
  static function phutil_loggable_string($string) {
   ...
   */
  static function phutil_fwrite_nonblocking_stream($stream, $bytes) {
    if (!strlen($bytes)) {
      return 0;
    }
   ...
   * ...with this:
   *
   *   $ttl = \Facebook\Libphutil\Functions\utils::phutil_units('30 days in seconds');
   *
   ...
   */
  static function phutil_units($description) {
   ...
   *                    failed to decode.
   */
  static function phutil_json_decode($string, $default = array()) {
   ...
   */
  static function phutil_censor_credentials($string) {
