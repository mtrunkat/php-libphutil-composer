...
class render {
   ...
   * @return \Facebook\Libphutil\PhutilSafeHTML Tag object.
   */
  static function phutil_tag($tag, array $attributes = array(), $content = null) {
    ...
    foreach ($attributes as $k => $v) {
      ...
      $v = \Facebook\Libphutil\Functions\render::phutil_escape_html($v);
      $attr_string .= ' '.$k.'="'.$v.'"';
    }
  
    if ($content === null) {
      if (isset($self_closing_tags[$tag])) {
        return new \Facebook\Libphutil\PhutilSafeHTML('<'.$tag.$attr_string.' />');
      } else {
        $content = '';
      }
    } else {
      $content = \Facebook\Libphutil\Functions\render::phutil_escape_html($content);
    }
  
    return new \Facebook\Libphutil\PhutilSafeHTML('<'.$tag.$attr_string.'>'.$content.'</'.$tag.'>');
  }
  
  /**
   * @group markup
   */
  static function phutil_tag_div($class, $content = null) {
    return \Facebook\Libphutil\Functions\render::phutil_tag('div', array('class' => $class), $content);
  }
  
  /**
   * @group markup
   */
  static function phutil_escape_html($string) {
    if ($string instanceof \Facebook\Libphutil\PhutilSafeHTML) {
      return $string;
    } else if ($string instanceof \Facebook\Libphutil\PhutilSafeHTMLProducerInterface) {
      $result = $string->producePhutilSafeHTML();
      if ($result instanceof \Facebook\Libphutil\PhutilSafeHTML) {
        return \Facebook\Libphutil\Functions\render::phutil_escape_html($result);
      } else if (is_array($result)) {
        return \Facebook\Libphutil\Functions\render::phutil_escape_html($result);
      } else if ($result instanceof \Facebook\Libphutil\PhutilSafeHTMLProducerInterface) {
        return \Facebook\Libphutil\Functions\render::phutil_escape_html($result);
      } else {
        try {
          \Facebook\Libphutil\Functions\utils::assert_stringlike($result);
          return \Facebook\Libphutil\Functions\render::phutil_escape_html((string)$result);
        } catch (\Exception $ex) {
          $class = get_class($string);
          throw new \Exception(
            "Object (of class '{$class}') implements ".
            "\Facebook\Libphutil\PhutilSafeHTMLProducerInterface but did not return anything ".
      ...
      foreach ($string as $item) {
        $result .= \Facebook\Libphutil\Functions\render::phutil_escape_html($item);
      }
      return $result;
    }
   ...
   * @group markup
   */
  static function phutil_escape_html_newlines($string) {
    return \Facebook\Libphutil\PhutilSafeHTML::applyFunction('nl2br', $string);
  }
   ...
   * @group markup
   */
  static function phutil_safe_html($string) {
    if ($string == '') {
      return $string;
    } else if ($string instanceof \Facebook\Libphutil\PhutilSafeHTML) {
      return $string;
    } else {
      return new \Facebook\Libphutil\PhutilSafeHTML($string);
    }
  }
   ...
   * @group markup
   */
  static function phutil_implode_html($glue, array $pieces) {
    $glue = \Facebook\Libphutil\Functions\render::phutil_escape_html($glue);
  
    foreach ($pieces as $k => $piece) {
      $pieces[$k] = \Facebook\Libphutil\Functions\render::phutil_escape_html($piece);
    }
  
    return \Facebook\Libphutil\Functions\render::phutil_safe_html(implode($glue, $pieces));
  }
   ...
   * @group markup
   */
  static function hsprintf($html/* , ... */) {
    $args = func_get_args();
    array_shift($args);
    return new \Facebook\Libphutil\PhutilSafeHTML(
      vsprintf($html, array_map('\Facebook\Libphutil\Functions\render::phutil_escape_html', $args)));
  }
   ...
   * these cases:
   *
   *    '/path/?param='.\Facebook\Libphutil\Functions\render::phutil_escape_uri($string);         # OK: Query Parameter
   *    '/path/to/'.\Facebook\Libphutil\Functions\render::phutil_escape_uri($string);             # OK: URI Suffix
   *
   * It will potentially produce the WRONG behavior in this special case:
   *
   *    COUNTEREXAMPLE
   *    '/path/to/'.\Facebook\Libphutil\Functions\render::phutil_escape_uri($string).'/thing/';   # BAD: URI Infix
   *
   ...
   * @group markup
   */
  static function phutil_escape_uri($string) {
   ...
   * @group markup
   */
  static function phutil_escape_uri_path_component($string) {
   ...
   * @group markup
   */
  static function phutil_unescape_uri_path_component($string) {
