...
class PhutilAuthAdapterOAuthGoogle extends \Facebook\Libphutil\PhutilAuthAdapterOAuth {
  ...
  public function getAccountID() {
    ...
    foreach ($emails as $email) {
      if (\Facebook\Libphutil\Functions\utils::idx($email, 'type') == 'account') {
        return \Facebook\Libphutil\Functions\utils::idx($email, 'value');
      }
    }
  ...
  public function getAccountName() {
    ...
    $email = explode('@', $email);
    $email = \Facebook\Libphutil\Functions\utils::head($email);
    return $email;
  }
  ...
  public function getAccountURI() {
    return 'https://plus.google.com/'.$this->getOAuthAccountData('\Facebook\Libphutil\Functions\utils::id');
  }
  ...
  public function getAccountRealName() {
    ...
    $parts = array();
    $parts[] = \Facebook\Libphutil\Functions\utils::idx($name, 'givenName');
    unset($name['givenName']);
    $parts[] = \Facebook\Libphutil\Functions\utils::idx($name, 'familyName');
  ...
  protected function loadOAuthAccountData() {
    $uri = new \Facebook\Libphutil\PhutilURI('https://www.googleapis.com/plus/v1/people/me');
  ...
  private function tryToThrowSpecializedError($status, $raw_body) {
    if (!($status instanceof \Facebook\Libphutil\HTTPFutureResponseStatusHTTP)) {
      return;
    }
    ...
    $body = \Facebook\Libphutil\Functions\utils::phutil_json_decode($raw_body);
    if (!$body) {
      return;
    }
    ...
    $domain = \Facebook\Libphutil\Functions\utils::idx($error, 'domain');
    $reason = \Facebook\Libphutil\Functions\utils::idx($error, 'reason');

    if ($domain == 'usageLimits' && $reason == 'accessNotConfigured') {
      throw new \Facebook\Libphutil\PhutilAuthConfigurationException(
        \Facebook\Libphutil\Functions\pht::pht(
