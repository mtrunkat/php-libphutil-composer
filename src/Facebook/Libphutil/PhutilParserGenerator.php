 ...
 *
 *    $parser = \Facebook\Libphutil\Functions\utils::id(new \Facebook\Libphutil\PhutilParserGenerator())
 ...
 *
 *    $tokens = get_tokens(); // Usually from \Facebook\Libphutil\PhutilLexer
...
/* -(  Rule Validation  )---------------------------------------------------- */
        ...
        if ($variant === null) {
          ...
          foreach ($variant as $symbol) {
            if (($symbol === null) && count($variant) > 1) {
              throw new \Facebook\Libphutil\PhutilParserGeneratorInvalidRuleException(
    ...
    foreach ($this->rules as $rule => $productions) {
      foreach ($productions as $production_name => $production) {
        foreach ($production as $symbol) {
          ...
          throw new \Facebook\Libphutil\PhutilParserGeneratorUnknownSymbolException(
  ...
  private function validateAllTerminalsReachable() {
    ...
    if ($missing) {
      ...
      throw new \Facebook\Libphutil\PhutilParserGeneratorUnreachableTerminalException(
  ...
  private function validateAllRulesReachable() {
    ...
    if ($missing) {
      ...
      throw new \Facebook\Libphutil\PhutilParserGeneratorUnreachableRuleException(
  ...
  private function validateAllRulesReducible() {
    ...
    foreach ($this->rules as $rule => $productions) {
      if (!$this->isRuleReducible($rule, $reducible)) {
        throw new \Facebook\Libphutil\PhutilParserGeneratorIrreducibleRuleException(
...
/* -(  Computing States  )--------------------------------------------------- */
  ...
  private function addState(array $set) {
    ...
    $this->states[] = $set;
    $state = \Facebook\Libphutil\Functions\utils::last_key($this->states);
  ...
  private function buildTables() {
    ...
    foreach ($this->states as $state => $items) {
      ...
      foreach ($reduce as $next => $reductions) {
        ...
        $reduce[$next] = \Facebook\Libphutil\Functions\utils::head($reductions);
      }
  ...
  public function generateParserFunction($name) {
    ...
    $out[] = '  return \Facebook\Libphutil\PhutilParserGenerator::parseTokensWithTables(';
  ...
  public static function parseTokensWithTables(
    ...
    return \Facebook\Libphutil\Functions\utils::head($token_stack);
  }
