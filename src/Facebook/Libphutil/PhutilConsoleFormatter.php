...
class PhutilConsoleFormatter {
  ...
  public static function getDisableANSI() {
    if (self::$disableANSI === null) {
      $term = \Facebook\Libphutil\Functions\utf8::phutil_utf8_strtolower(getenv("TERM"));
      if (\Facebook\Libphutil\Functions\utils::phutil_is_windows() && $term !== "cygwin" && $term !== "ansi") {
  ...
  public static function formatString($format /* ... */) {
    ...
    if (self::getDisableANSI()) {
      ...
      $format = preg_replace_callback(
        '@<(fg|bg):('.$colors.')>(.*)</\1>@sU',
        array('\Facebook\Libphutil\PhutilConsoleFormatter', 'replaceColorCode'),
        $format);
    }
