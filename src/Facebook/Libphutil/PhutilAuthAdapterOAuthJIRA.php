...
final class PhutilAuthAdapterOAuthJIRA extends \Facebook\Libphutil\PhutilAuthAdapterOAuth1 {
  ...
  public function getAccountID() {
    ...
    return \Facebook\Libphutil\Functions\utils::idx($this->getUserInfo(), 'key');
  }

  public function getAccountName() {
    return \Facebook\Libphutil\Functions\utils::idx($this->getUserInfo(), 'name');
  }

  public function getAccountImageURI() {
    $avatars = \Facebook\Libphutil\Functions\utils::idx($this->getUserInfo(), 'avatarUrls');
    if ($avatars) {
      return \Facebook\Libphutil\Functions\utils::idx($avatars, '48x48');
    }
    return null;
  }

  public function getAccountRealName() {
    return \Facebook\Libphutil\Functions\utils::idx($this->getUserInfo(), 'displayName');
  }

  public function getAccountEmail() {
    return \Facebook\Libphutil\Functions\utils::idx($this->getUserInfo(), 'emailAddress');
  }
  ...
  protected function willFinishOAuthHandshake() {
    ...
    if ($this->getVerifier() == $jira_magic_word) {
      throw new \Facebook\Libphutil\PhutilAuthUserAbortedException();
    }
  }

  public function newJIRAFuture($path, $method, $params = array()) {
    $uri = new \Facebook\Libphutil\PhutilURI($this->getJIRAURI($path));
    if ($method == 'GET') {
