...
/**
 * Query and load Phutil classes, interfaces and functions. \Facebook\Libphutil\PhutilSymbolLoader
 ...
 *
 *    $symbols = \Facebook\Libphutil\Functions\utils::id(new \Facebook\Libphutil\PhutilSymbolLoader())
 *      ->setType('class')
 *      ->setLibrary('example')
 ...
 *
 * NOTE: This class must not use libphutil funtions, including \Facebook\Libphutil\Functions\utils::id() and \Facebook\Libphutil\Functions\utils::idx().
 *
...
/* -(  Load  )--------------------------------------------------------------- */
  ...
  public function loadObjects(array $argv = array()) {
    ...
    $objects = array();
    foreach ($symbols as $symbol) {
      $objects[$symbol['name']] = \Facebook\Libphutil\Functions\utils::newv($symbol['name'], $argv);
    }
...
/* -(  Internals  )---------------------------------------------------------- */
  ...
  private function loadSymbol(array $symbol_spec) {
    ...
    if ($load_failed !== null) {
      $lib_path = \Facebook\Libphutil\Functions\moduleutils::phutil_get_library_root($lib_name);
