...
class HTTPFuture extends \Facebook\Libphutil\BaseHTTPFuture {
  ...
  private function buildSocket() {
    ...
    if (!$socket) {
      ...
      $this->result = $this->buildErrorResult(
        \Facebook\Libphutil\HTTPFutureResponseStatusTransport::ERROR_CONNECTION_FAILED);
      return null;
    }
  ...
  private function checkSocket() {
    ...
    if ($timeout) {
      $this->result = $this->buildErrorResult(
        \Facebook\Libphutil\HTTPFutureResponseStatusTransport::ERROR_TIMEOUT);
    } else if (!$this->stateConnected) {
      $this->result = $this->buildErrorResult(
        \Facebook\Libphutil\HTTPFutureResponseStatusTransport::ERROR_CONNECTION_REFUSED);
  ...
  private function buildErrorResult($error) {
    return array(
      $status = new \Facebook\Libphutil\HTTPFutureResponseStatusTransport($error, $this->getURI()),
      $body = null,
      $headers = array());
  }
