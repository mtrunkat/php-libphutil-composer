...
class PhutilAuthAdapterLDAP extends \Facebook\Libphutil\PhutilAuthAdapter {
  ...
  public function setAnonymousPassword(
    \Facebook\Libphutil\PhutilOpaqueEnvelope $anonymous_password) {
  ...
  public function setLoginPassword(\Facebook\Libphutil\PhutilOpaqueEnvelope $login_password) {
  ...
  public function readLDAPRecordAccountID(array $record) {
    ...
    if (!strlen($key)) {
      $key = \Facebook\Libphutil\Functions\utils::head($this->searchAttributes);
    }
  ...
  private function readLDAPData(array $data, $key, $default = null) {
    $list = \Facebook\Libphutil\Functions\utils::idx($data, $key);
    if ($list === null) {
      ...
      $list = \Facebook\Libphutil\Functions\utils::idx($data, \Facebook\Libphutil\Functions\utf8::phutil_utf8_strtolower($key));
    }
  ...
  private function formatLDAPAttributeSearch($attribute, $login_user) {
    ...
    if (strpos($attribute, '${login}') !== false) {
      $escaped_user = \Facebook\Libphutil\Functions\ldapsprintf::ldap_sprintf('%S', $login_user);
    ...
    return \Facebook\Libphutil\Functions\ldapsprintf::ldap_sprintf(
      '%Q=%S',
      $attribute,
      $login_user);
  }
  ...
  private function loadLDAPUserData() {
    ...
    if ($this->shouldBindWithoutIdentity()) {
      ...
      if ($distinguished_name === null) {
        throw new \Facebook\Libphutil\PhutilAuthCredentialException();
      }
    } else {
      $search_query = $this->formatLDAPAttributeSearch(
        \Facebook\Libphutil\Functions\utils::head($this->searchAttributes),
        $login_user);
      if ($this->activeDirectoryDomain) {
        $distinguished_name = \Facebook\Libphutil\Functions\ldapsprintf::ldap_sprintf(
          '%s@%Q',
          $login_user,
    ...
    if (!$this->ldapConnection) {
      ...
      if ($this->ldapStartTLS) {
        ...
        if (!$ok) {
          $this->raiseConnectionException(
            $conn,
            \Facebook\Libphutil\Functions\pht::pht("Unable to start TLS connection when connecting to LDAP."));
        }
      }
  ...
  private function searchLDAPForRecord($dn) {
    ...
    return \Facebook\Libphutil\Functions\utils::head($results);
  }

  public function searchLDAP($pattern /* ... */) {
    $args = func_get_args();
    $query = call_user_func_array('\Facebook\Libphutil\Functions\ldapsprintf::ldap_sprintf', $args);
    ...
    if (!$result) {
      $this->raiseConnectionException(
        $conn,
        \Facebook\Libphutil\Functions\pht::pht("LDAP search failed."));
    }
    ...
    if (!$entries) {
      $this->raiseConnectionException(
        $conn,
        \Facebook\Libphutil\Functions\pht::pht("Failed to get LDAP entries from search result."));
    }
  ...
  private function raiseConnectionException($conn, $message) {
    ...
    if ($errno || $error) {
      $full_message = \Facebook\Libphutil\Functions\pht::pht(
        "LDAP Exception: %s\nLDAP Error #%d: %s",
        $message,
        $errno,
        $error);
  ...
  private function bindLDAP($conn, $user, \Facebook\Libphutil\PhutilOpaqueEnvelope $pass) {
    $profiler = \Facebook\Libphutil\PhutilServiceProfiler::getInstance();
    $call_id = $profiler->beginServiceCall(
      array(
        'type' => 'ldap',
    ...
    if (!$ok) {
      if (strlen($user)) {
        $this->raiseConnectionException(
          $conn,
          \Facebook\Libphutil\Functions\pht::pht('Failed to bind to LDAP server (as user "%s").', $user));
      } else {
        $this->raiseConnectionException(
          $conn,
          \Facebook\Libphutil\Functions\pht::pht('Failed to bind to LDAP server (without username).'));
      }
    }
  }
