 ...
 * @group markup
 */
class PhutilRemarkupEngineRemarkupListBlockRule
  ...
  public function markupText($text, $children) {
    ...
    return \Facebook\Libphutil\Functions\render::phutil_implode_html('', $out);
  }
  ...
  private function renderTree(array $tree, $level, $has_marks) {
    $style = \Facebook\Libphutil\Functions\utils::idx(\Facebook\Libphutil\Functions\utils::head($tree), 'style');
    ...
    if (!$this->getEngine()->isTextMode()) {
      ...
      if ($has_marks) {
        $out[] = \Facebook\Libphutil\Functions\render::hsprintf('<%s class="remarkup-list-with-checkmarks">', $tag);
      } else {
        $out[] = \Facebook\Libphutil\Functions\render::hsprintf('<%s>', $tag);
      }
    ...
    foreach ($tree as $item) {
      if ($this->getEngine()->isTextMode()) {
        ...
        $out[] = \Facebook\Libphutil\Functions\render::hsprintf('<li class="phantom-item">');
      } else {
        if ($item['mark'] !== null) {
          if ($item['mark'] == true) {
            $out[] = \Facebook\Libphutil\Functions\render::hsprintf('<li class="remarkup-checked-item">');
          } else {
            $out[] = \Facebook\Libphutil\Functions\render::hsprintf('<li class="remarkup-unchecked-item">');
          }
          $out[] = \Facebook\Libphutil\Functions\render::phutil_tag(
            'input',
            array(
              'type' => 'checkbox',
          ...
          $out[] = \Facebook\Libphutil\Functions\render::hsprintf('<li>');
        }
    ...
    if (!$this->getEngine()->isTextMode()) {
      switch ($style) {
        case '#':
          $out[] = \Facebook\Libphutil\Functions\render::hsprintf('</ol>');
          break;
        case '-':
          $out[] = \Facebook\Libphutil\Functions\render::hsprintf('</ul>');
          break;
      }
    }
