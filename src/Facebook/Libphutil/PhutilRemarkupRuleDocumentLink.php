 ...
 * @group markup
 */
class PhutilRemarkupRuleDocumentLink
  ...
  public function markupDocumentLink($matches) {
    $uri = trim($matches[1]);
    $name = trim(\Facebook\Libphutil\Functions\utils::idx($matches, 2, $uri));
    ...
    if ($is_uri && strncmp('/', $uri, 1) && strncmp('#', $uri, 1)) {
      $protocols = $this->getEngine()->getConfig(
        ...
        array());
      $protocol = \Facebook\Libphutil\Functions\utils::id(new \Facebook\Libphutil\PhutilURI($uri))->getProtocol();
