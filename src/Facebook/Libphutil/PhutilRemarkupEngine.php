...
final class PhutilRemarkupEngine extends \Facebook\Libphutil\PhutilMarkupEngine {
  ...
  public function getConfig($key, $default = null) {
    return \Facebook\Libphutil\Functions\utils::idx($this->config, $key, $default);
  }
  ...
  public function setBlockRules(array $rules) {
    \Facebook\Libphutil\Functions\utils::assert_instances_of($rules, '\Facebook\Libphutil\PhutilRemarkupEngineBlockRule');

    $rules = \Facebook\Libphutil\Functions\utils::msort($rules, 'getPriority');
  ...
  public function getTextMetadata($key, $default = null) {
    ...
    return \Facebook\Libphutil\Functions\utils::idx($this->metadata, $key, $default);
  }
  ...
  public function storeText($text) {
    if ($this->isTextMode()) {
      $text = \Facebook\Libphutil\Functions\render::phutil_safe_html($text);
    }
  ...
  public function overwriteStoredText($token, $new_text) {
    if ($this->isTextMode()) {
      $new_text = \Facebook\Libphutil\Functions\render::phutil_safe_html($new_text);
    }
  ...
  public function preprocessText($text) {
    $this->metadata = array();
    $this->storage = new \Facebook\Libphutil\PhutilRemarkupBlockStorage();
  ...
  private function splitTextIntoBlocks($text, $depth = 0) {
    ...
    while (isset($text[$cursor])) {
      ...
      foreach ($block_rules as $block_rule) {
        ...
        if ($num_lines) {
          if ($blocks) {
            $prev_block = \Facebook\Libphutil\Functions\utils::last($blocks);
          }
          ...
          if ($prev_block
            ...
            $blocks[\Facebook\Libphutil\Functions\utils::last_key($blocks)]["num_lines"] += $curr_block["num_lines"];
            $blocks[\Facebook\Libphutil\Functions\utils::last_key($blocks)]["is_empty"] =
              $blocks[\Facebook\Libphutil\Functions\utils::last_key($blocks)]["is_empty"] && $curr_block["is_empty"];
          } else {
            $blocks[] = $curr_block;
          }
  ...
  private function flattenOutput(array $output) {
    if ($this->isTextMode()) {
      ...
      $output = \Facebook\Libphutil\Functions\render::phutil_implode_html("\n\n", $output);
    }
  ...
  private static function shouldMergeBlocks($text, $prev_block, $curr_block) {
    $block_rules = \Facebook\Libphutil\Functions\utils::ipull(array($prev_block, $curr_block), "rule");

    $default_rule = "\Facebook\Libphutil\PhutilRemarkupEngineRemarkupDefaultBlockRule";
    try {
      \Facebook\Libphutil\Functions\utils::assert_instances_of($block_rules, $default_rule);
  ...
  public function postprocessText(array $dict) {
    $this->metadata = \Facebook\Libphutil\Functions\utils::idx($dict, 'metadata', array());

    $this->storage = new \Facebook\Libphutil\PhutilRemarkupBlockStorage();
    $this->storage->setMap(\Facebook\Libphutil\Functions\utils::idx($dict, 'storage', array()));
    ...
    return $this->restoreText(\Facebook\Libphutil\Functions\utils::idx($dict, 'output'), $this->isTextMode());
  }
