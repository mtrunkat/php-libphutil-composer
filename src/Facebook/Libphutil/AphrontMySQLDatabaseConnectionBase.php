...
abstract class AphrontMySQLDatabaseConnectionBase
  ...
  protected function getConfiguration($key, $default = null) {
    return \Facebook\Libphutil\Functions\utils::idx($this->configuration, $key, $default);
  }
    ...
    while ($retries--) {
      try {
        ...
        if ($retries && $ex->getCode() == 2003) {
          ...
          \Facebook\Libphutil\Functions\phlog::phlog("Retrying ({$retries}) after {$class}: {$message}");
        } else {
          $profiler->endServiceCall($call_id, array());
          throw $ex;
        }
  ...
  public function executeRawQuery($raw_query) {
    ...
    while ($retries--) {
      ...
      } catch (\Facebook\Libphutil\AphrontQueryConnectionLostException $ex) {
  ...
  protected function throwQueryCodeException($errno, $error) {
    ...
    switch ($errno) {
      case 2013: // Connection Dropped
        throw new \Facebook\Libphutil\AphrontQueryConnectionLostException($exmsg);
      case 2006: // Gone Away
      ...
      case 1205: // Lock wait timeout exceeded
        throw new \Facebook\Libphutil\AphrontQueryDeadlockException($exmsg);
      case 1062: // Duplicate Key
        ...
        // exception.
        throw new \Facebook\Libphutil\AphrontQueryDuplicateKeyException($exmsg);
      case 1044: // Access denied to database
      case 1045: // Access denied (auth)
      case 1142: // Access denied to table
      case 1143: // Access denied to column
        throw new \Facebook\Libphutil\AphrontQueryAccessDeniedException($exmsg);
      case 1146: // No such table
      case 1049: // No such database
      case 1054: // Unknown column "..." in field list
        throw new \Facebook\Libphutil\AphrontQuerySchemaException($exmsg);
      default:
        // TODO: 1064 is syntax error, and quite terrible in production.
        throw new \Facebook\Libphutil\AphrontQueryException($exmsg);
    }
  }
  ...
  protected function validateUTF8String($string) {
    if (\Facebook\Libphutil\Functions\utf8::phutil_is_utf8_with_only_bmp_characters($string)) {
      return;
    }

    throw new \Facebook\Libphutil\AphrontQueryCharacterSetException(
