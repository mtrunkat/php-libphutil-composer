 ...
 *
 *   $future = new \Facebook\Libphutil\ExecFuture('hg log ...');
 *   foreach (new \Facebook\Libphutil\LinesOfALargeExecFuture($future) as $line) {
 *     // ...
 *   }
 *
...
/* -(  Construction  )------------------------------------------------------- */
   ...
   *
   * @param \Facebook\Libphutil\ExecFuture \Facebook\Libphutil\Future to wrap.
   * @return void
   * @task construct
   */
...
/* -(  Internals  )---------------------------------------------------------- */
  ...
  protected function willRewind() {
    if ($this->didRewind) {
      throw new \Exception(
        "You can not reiterate over a \Facebook\Libphutil\LinesOfALargeExecFuture object. The ".
