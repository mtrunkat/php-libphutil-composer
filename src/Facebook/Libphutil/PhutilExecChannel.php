 ...
 *
 *   $future = new \Facebook\Libphutil\ExecFuture('nc example.com 80');
 *   $channel = new \Facebook\Libphutil\PhutilExecChannel($future);
 *
 *   $channel->write("GET / HTTP/1.0\n\n");
 *   while (true) {
 *     echo $channel->read();
 *
 *     \Facebook\Libphutil\PhutilChannel::waitForAny(array($channel));
 *     if (!$channel->update()) {
...
/* -(  Construction  )------------------------------------------------------- */
   ...
   *
   * @param \Facebook\Libphutil\ExecFuture \Facebook\Libphutil\Future to use as an underlying I/O source.
   * @task construct
   */
  public function __construct(\Facebook\Libphutil\ExecFuture $future) {
    parent::__construct();
  ...
  protected function writeBytes($bytes) {
    throw new \Exception("\Facebook\Libphutil\ExecFuture can not write bytes directly!");
  }
  ...
  public function setReadBufferSize($size) {
    ...
    // \Facebook\Libphutil\ExecFuture and one inside the Channel. We could tune this eventually, but
   ...
   *
   *   function f(\Facebook\Libphutil\PhutilExecChannel $channel, $stderr) {
   *     // ...
   *   }
   *
