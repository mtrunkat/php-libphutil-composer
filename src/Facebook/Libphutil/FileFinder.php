 ...
 *    $files = \Facebook\Libphutil\Functions\utils::id(new \Facebook\Libphutil\FileFinder('/tmp'))
 *      ->withType('f')
 *      ->withSuffix('php')
 *      ->find();
 *
...
class FileFinder {
  ...
  private function getFiles($dir) {
    $found = \Facebook\Libphutil\Filesystem::listDirectory($this->root.'/'.$dir, true);
    $files = array();
    if (strlen($dir) > 0) {
      $dir = rtrim($dir, '/').'/';
    }
  ...
  public function find() {
    ...
    if (!is_dir($this->root) || !is_readable($this->root)) {
      throw new \Exception(
        "Invalid \Facebook\Libphutil\FileFinder root directory specified ('{$this->root}'). ".
    ...
    if ($this->forceMode == "shell") {
      ...
      $php_mode = (\Facebook\Libphutil\Functions\utils::phutil_is_windows() || !\Facebook\Libphutil\Filesystem::binaryExists('find'));
    }
    ...
    if ($php_mode) {
      ...
      list($stdout) = \Facebook\Libphutil\Functions\utils::newv('\Facebook\Libphutil\ExecFuture', $args)
        ->setCWD($this->root)
        ->resolvex();
