...
function phutil_register_library($library, $path) {
  \Facebook\Libphutil\PhutilBootloader::getInstance()->registerLibrary($library, $path);
}
...
function phutil_register_library_map(array $map) {
  \Facebook\Libphutil\PhutilBootloader::getInstance()->registerLibraryMap($map);
}
...
function phutil_load_library($path) {
  \Facebook\Libphutil\PhutilBootloader::getInstance()->loadLibrary($path);
}
...
class PhutilBootloader {
  ...
  public static function getInstance() {
    if (!self::$instance) {
      self::$instance = new \Facebook\Libphutil\PhutilBootloader();
    }
  ...
  public function registerLibrary($name, $path) {
    ...
    if (!class_exists('\Facebook\Libphutil\PhutilSymbolLoader', false)) {
      $root = $this->getLibraryRoot('phutil');
      $this->executeInclude($root.'/symbols/PhutilSymbolLoader.php');
    }

    $loader = new \Facebook\Libphutil\PhutilSymbolLoader();
    $loader
      ->setLibrary($name)
      ->setType('function');
    ...
    } catch (\Facebook\Libphutil\PhutilMissingSymbolException $ex) {
    ...
    if (empty($_SERVER['PHUTIL_DISABLE_RUNTIME_EXTENSIONS'])) {
      ...
      if (\Facebook\Libphutil\Filesystem::pathExists($extdir)) {
        $extensions = \Facebook\Libphutil\Functions\utils::id(new \Facebook\Libphutil\FileFinder($extdir))
          ->withSuffix('php')
          ->withType('f')
  ...
  public function getLibraryMap($name) {
    if (empty($this->libraryMaps[$name])) {
      ...
      // NOTE: We can't use "\Facebook\Libphutil\Functions\utils::idx()" here because it may not be loaded yet.
  ...
  private function loadExtension($library, $root, $path) {
    ...
    foreach ($add_functions as $func => $ignored) {
      ...
      $fpath = \Facebook\Libphutil\Filesystem::resolvePath($rfunc->getFileName(), $root);
      $this->libraryMaps[$library]['function'][$func] = $fpath;
    }

    foreach ($add_classes + $add_interfaces as $class => $ignored) {
      $rclass = new \ReflectionClass($class);
      $cpath = \Facebook\Libphutil\Filesystem::resolvePath($rclass->getFileName(), $root);
      ...
      if ($xmap) {
        ...
        if (count($xmap) == 1) {
          $xmap = \Facebook\Libphutil\Functions\utils::head($xmap);
        }
...
function __phutil_autoload($class_name) {
  ...
  try {
    ...
    if (!$symbols) {
      throw new \Facebook\Libphutil\PhutilMissingSymbolException(
        $class_name,
        'class or interface',
