 ...
 */
abstract class PhutilAuthAdapterOAuth1 extends \Facebook\Libphutil\PhutilAuthAdapter {
  ...
  public function setPrivateKey(\Facebook\Libphutil\PhutilOpaqueEnvelope $private_key) {
  ...
  public function setConsumerSecret(\Facebook\Libphutil\PhutilOpaqueEnvelope $consumer_secret) {
  ...
  protected function newOAuth1Future($uri, $data = array()) {
    $future = \Facebook\Libphutil\Functions\utils::id(new \Facebook\Libphutil\PhutilOAuth1Future($uri, $data))
      ->setMethod('POST')
  ...
  public function getClientRedirectURI() {
    ...
    $data = \Facebook\Libphutil\Functions\utils::id(new \Facebook\Libphutil\PhutilQueryStringParser())->parseQueryString($body);

    // NOTE: Per the spec, this value MUST be the string 'true'.
    $confirmed = \Facebook\Libphutil\Functions\utils::idx($data, 'oauth_callback_confirmed');
    if ($confirmed !== "true") {
      throw new \Exception(
    ...
    $authorize_token_uri = new \Facebook\Libphutil\PhutilURI($this->getAuthorizeTokenURI());
  ...
  private function readTokenAndTokenSecret(array $data) {
    $token = \Facebook\Libphutil\Functions\utils::idx($data, 'oauth_token');
    if (!$token) {
      throw new \Exception("Expected 'oauth_token' in response!");
    }

    $token_secret = \Facebook\Libphutil\Functions\utils::idx($data, 'oauth_token_secret');
    if (!$token_secret) {
