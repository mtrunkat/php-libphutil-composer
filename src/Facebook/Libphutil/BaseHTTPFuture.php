 ...
 *
 *   $future = new \Facebook\Libphutil\HTTPFuture('http://www.example.com/');
 ...
 * @group futures
 */
abstract class BaseHTTPFuture extends \Facebook\Libphutil\Future {
...
/* -(  Configuring the Request  )-------------------------------------------- */
   ...
   * @param \Facebook\Libphutil\PhutilOpaqueEnvelope  Password to authenticate with.
   * @return this
   * @task config
   */
  public function setHTTPBasicAuthCredentials(
    $username,
    \Facebook\Libphutil\PhutilOpaqueEnvelope $password) {
...
/* -(  Internals  )---------------------------------------------------------- */
  ...
  protected function parseRawHTTPResponse($raw_response) {
    ...
    while (true) {
      ...
      if ($response_code == 100) {
        ...
        $headers = $this->parseHeaders(\Facebook\Libphutil\Functions\utils::idx($matches, 'headers'));
        break;
      }
    }

    $status = new \Facebook\Libphutil\HTTPFutureResponseStatusHTTP(
      $response_code,
      $body,
      $headers,
      $this->expect);
  ...
  public static function getHeader(array $headers, $search) {
    \Facebook\Libphutil\Functions\utils::assert_instances_of($headers, 'array');
  ...
  protected function buildMalformedResult($raw_response) {
    ...
    $status = new \Facebook\Libphutil\HTTPFutureResponseStatusParse(
      \Facebook\Libphutil\HTTPFutureResponseStatusParse::ERROR_MALFORMED_RESPONSE,
      $raw_response);
    return array($status, $body, $headers);
  }
}
