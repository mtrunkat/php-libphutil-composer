 ...
 *
 *   if (!$success) {
 *     throw new \Facebook\Libphutil\PhutilAggregateException("All engines failed:", $exceptions);
 *   }
 *
 * @concrete-extensible
 * @group error
...
class PhutilAggregateException extends \Exception {
  ...
  public function __construct($message, array $other_exceptions) {
    // We don't call \Facebook\Libphutil\Functions\utils::assert_instances_of($other_exceptions, '\Exception') to not
