 ...
 *
 * \Facebook\Libphutil\ExecFuture is a future, which means it runs asynchronously and represents
 * a value which may not exist yet. See @{article:Using Futures} for an
 * explanation of futures. When an \Facebook\Libphutil\ExecFuture resolves, it returns the exit
 * code, stdout and stderr of the process it executed.
 *
 * \Facebook\Libphutil\ExecFuture is the core command execution implementation in libphutil, but is
...
/* -(  Creating ExecFutures  )----------------------------------------------- */
   ...
   *
   *   $future = new \Facebook\Libphutil\ExecFuture('wc -l %s', $file_path);
   *
   ...
   * @return \Facebook\Libphutil\ExecFuture ExecFuture for running the specified command.
   * @task create
   */
  public function __construct($command) {
    $argv = func_get_args();
    $this->command = call_user_func_array('\Facebook\Libphutil\Functions\csprintf::csprintf', $argv);
    $this->stdin = new \Facebook\Libphutil\PhutilRope();
  }
...
/* -(  Interacting With Commands  )------------------------------------------ */
   ...
   * previously read:
   *
   *   $future = new \Facebook\Libphutil\ExecFuture('...');
   *   // ...
  ...
  public function write($data, $keep_pipe = false) {
    if (strlen($data)) {
      if (!$this->stdin) {
        throw new \Exception(\Facebook\Libphutil\Functions\pht::pht('Writing to a closed pipe!'));
      }
   ...
   *
   *   $future = new \Facebook\Libphutil\ExecFuture('zcat huge_file.gz');
   *   do {
   ...
   * \Facebook\Libphutil\ExecFuture and ##netcat##, but you probably should not do that.
   *
...
/* -(  Resolving Execution  )------------------------------------------------ */
  ...
  public function resolvex($timeout = null) {
    ...
    if ($err) {
      $cmd = $this->command;
      throw new \Facebook\Libphutil\CommandException(
...
/* -(  Internals  )---------------------------------------------------------- */
  ...
  public function isReady() {
    // NOTE: We have soft dependencies on \Facebook\Libphutil\PhutilServiceProfiler and
    // \Facebook\Libphutil\PhutilErrorTrap here. These depencies are soft to avoid the need to
    ...
    if (!$this->pipes) {
      ...
      if (class_exists('\Facebook\Libphutil\PhutilServiceProfiler')) {
      ...
      if ($unmasked_command instanceof \Facebook\Libphutil\PhutilCommandString) {
      ...
      // NOTE: See note above about Phage.
      if (class_exists('\Facebook\Libphutil\PhutilErrorTrap')) {
        $trap = new \Facebook\Libphutil\PhutilErrorTrap();
      } else {
        $trap = null;
      }
  ...
  private function closeProcess() {
    ...
    if ($this->profilerCallID !== null) {
      ...
      $profiler->endServiceCall(
        $this->profilerCallID,
        array(
          'err' => $this->result ? \Facebook\Libphutil\Functions\utils::idx($this->result, 0) : null,
        ));
