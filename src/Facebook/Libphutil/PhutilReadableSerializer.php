    ...
    if (is_object($value)) {
      ...
      if ($value) {
        ...
        $str .= '{ '.self::printShort(key($value)).' => '.
          self::printShort(\Facebook\Libphutil\Functions\utils::head($value)).' }';
      }
      return $str;
    } else {
      // NOTE: Avoid \Facebook\Libphutil\Functions\utf8::phutil_utf8_shorten() here since the data may not be
