...
/* -(  Key-Value Cache Implementation  )------------------------------------- */
  ...
  public function destroyCache() {
    \Facebook\Libphutil\Filesystem::remove($this->getCacheFile());
    return $this;
  }
...
/* -(  Cache Storage  )------------------------------------------------------ */
  ...
  private function loadCache($hold_lock) {
    ...
    $lock = \Facebook\Libphutil\PhutilFileLock::newForPath($this->getCacheFile().'.lock');
    try {
      $lock->lock($this->wait);
    ...
    try {
      $this->cache = array();
      if (\Facebook\Libphutil\Filesystem::pathExists($this->getCacheFile())) {
        $cache = unserialize(\Facebook\Libphutil\Filesystem::readFile($this->getCacheFile()));
        if ($cache) {
          $this->cache = $cache;
        }
      }
  ...
  private function saveCache() {
    ...
    \Facebook\Libphutil\Filesystem::writeFile($new, serialize($this->cache));
    \Facebook\Libphutil\Filesystem::rename($new, $this->getCacheFile());
