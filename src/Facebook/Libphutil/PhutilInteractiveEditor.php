 ...
 *
 *   $result = \Facebook\Libphutil\Functions\utils::id(new InteractiveEditor($document))
...
/* -(  Editing Interactively )----------------------------------------------- */
  ...
  public function editInteractively() {
    ...
    $tmp = \Facebook\Libphutil\Filesystem::createTemporaryDirectory('edit.');
    $path = $tmp.DIRECTORY_SEPARATOR.$name;

    try {
      \Facebook\Libphutil\Filesystem::writeFile($path, $content);
    } catch (\Exception $ex) {
      \Facebook\Libphutil\Filesystem::remove($tmp);
      throw $ex;
    }
    ...
    try {
      $result = \Facebook\Libphutil\Filesystem::readFile($path);
      \Facebook\Libphutil\Filesystem::remove($tmp);
    } catch (\Exception $ex) {
      \Facebook\Libphutil\Filesystem::remove($tmp);
      throw $ex;
  ...
  private function invokeEditor($editor, $path, $offset) {
    ...
    $offset_flag = '';
    if ($offset && !\Facebook\Libphutil\Functions\utils::phutil_is_windows()) {
      $offset = (int)$offset;
      if (preg_match('/^mate/', $editor)) {
        $offset_flag = \Facebook\Libphutil\Functions\csprintf::csprintf('-l %d', $offset);
      } else {
        $offset_flag = \Facebook\Libphutil\Functions\csprintf::csprintf('+%d', $offset);
      }
    }

    $cmd = \Facebook\Libphutil\Functions\csprintf::csprintf(
      '%C %C %s',
      $editor,
      $offset_flag,
      $path);

    return \Facebook\Libphutil\Functions\execx::phutil_passthru('%C', $cmd);
  }
...
/* -(  Configuring Options )------------------------------------------------- */
  ...
  public function getEditor() {
    ...
    if (\Facebook\Libphutil\Filesystem::binaryExists('editor')) {
      return 'editor';
    }
    ...
    if (\Facebook\Libphutil\Filesystem::binaryExists('nano')) {
      return 'nano';
    }
